...
- Create (screen 2) → aspect 9:16 / 16:9 / 1:1, Android system multi-document picker, camera capture (app-owned FileProvider URI), ordered selected media thumbnails, then import into the *real* project asset bin and append clips to the real timeline. No runtime storage permission needed. Never proceed on zero assets.
