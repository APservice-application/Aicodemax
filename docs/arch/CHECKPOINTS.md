...
| CP-144 | Built-in AI (HARD REQUIREMENT แก้250969) | TODO → DONE | audit (`docs/BUILTIN_AI_AUDIT.md`); FIX double-ChatML (สาเหตุ AI ใช้ไม่ได้) + empty=Failure; CI แพ็ก Qwen2.5-0.5B เป็น asset (~410MB APK) + host smoke test จริง; provision แตกไฟล์ครั้งแรก + `loadBuiltin` (ไม่ผ่าน ModelManager); การ์ดสถานะแทนปุ่มโหลด; Models ศูนย์ built-in/เสริม; About attribution |
