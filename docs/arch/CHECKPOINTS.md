...
| CP-147 | Qwen3-4B brain + URL/Intent + tool schemas + browser setup | TODO → DONE | model swap 0.5B→Qwen3-4B (2.5GB, `/no_think`, stripThinking) + `UrlResolver`/`IntentResolver`/`SearchEngines` + `StandardToolSchemas` 38 tools (bound/unbound จริง) + executor `search`/`info`/`current` + `AgentLoop` verify hook + new-tab sheet แทน auto-DDG + `docs/product/*matrix.md` |
