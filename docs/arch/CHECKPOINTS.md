...
| CP-145 | Local-build AI repair + diagnostics | TODO → DONE | `BuiltinAiReport` checklist (native/ABI/asset/โมเดลที่แตกไว้) แทนข้อความ "Android only" ที่ชวนงง; `scripts/fetch-builtin-model.sh` + `scripts/build-native-local.sh` + `docs/LOCAL_BUILD.md`; gitignore `jniLibs/` |
