...
| CP-142 | Image layers (§47) | TODO → DONE | `LayerStack` (add/select/rename/duplicate/delete/merge-down/visibility/lock) + `ImageOps.flatten` (source-over) + `image.flatten` tool; layer panel bottom-sheet (portrait) / side-panel (landscape); tap select, long-press menu; tools run on selected layer |
