...
| CP | ชื่อ | สถานะ | หลักฐาน/เหลือ |
|----|------|--------|----------------|
| CP-06 | Native/Local AI Runtime | DONE (via CP-144) | แถวเดิมตกยุค: มีโมเดลจริงแล้ว — Qwen2.5-0.5B Q4_K_M ฝังใน APK + JNI llama.cpp + smoke test ตอบไทยจริง 57.4 t/s |
...
| CP-12 | Agent Runtime | DONE (via CP-69) | specialists ครบ: File Agent (files+editor) + Shell Agent (terminal) ผ่าน `AllowlistedAgentRunner` + `AgentRegistry` |
...
| CP-130 | P14–16 validator + retry + few-shot + GBNF | DONE | ToolCallValidator live in gateway; ValidationRetry; FewShotStore; GbnfExport (CI GREEN) |
...
| CP-142 | Image layers (§47) | DONE + CI GREEN ทั้ง 2 สาขา (`edf32d9`) | `LayerStack` (add/select/rename/duplicate/delete/merge-down/visibility/lock) + `ImageOps.flatten` (source-over) + `image.flatten` tool; layer panel bottom-sheet (portrait) / side-panel (landscape); tap select, long-press menu; tools run on selected layer |
| CP-143 | Full terminal: persistent shell + live console | DONE | `PersistentShellPort` (one sh per session, cd/export persist, marker protocol, merged streams); CompatEngine port-session bridge (fixes UI exec); live transcript + Stop + cwd/timeout/history in TerminalScreen (CI GREEN) |
| CP-144 | Built-in AI (HARD REQUIREMENT แก้250969) | DONE + CI GREEN ทั้ง 2 สาขา (`307d661`) | audit (`docs/BUILTIN_AI_AUDIT.md`); FIX double-ChatML (สาเหตุ AI ใช้ไม่ได้) + empty=Failure; CI แพ็ก Qwen2.5-0.5B-Q4_K_M (491MB, Apache-2.0) เป็น asset + host smoke test ตอบ `สวัสดี` จริง (57.4 t/s); provision แตกไฟล์ครั้งแรก + `loadBuiltin` (ไม่ผ่าน ModelManager); การ์ดสถานะแทนปุ่มโหลด; Models ศูนย์ built-in/เสริม; About attribution; kc 748/748 |
| CP-145 | Local-build AI repair + diagnostics | DONE + CI GREEN ทั้ง 2 สาขา (`0a15322`) | `BuiltinAiReport` checklist (native/ABI/asset/โมเดลที่แตกไว้) แทนข้อความ "Android only" ที่ชวนงง; `scripts/fetch-builtin-model.sh` + `scripts/build-native-local.sh` + `docs/LOCAL_BUILD.md`; gitignore `jniLibs/` |
| CP-146 | Build-time AI embedding (ONE APP + ONE BUILT-IN AI) | DONE + CI GREEN ทั้ง 2 สาขา (`be20c1c`) | `CMakeLists` + `externalNativeBuild` (llama v0.4.1/whisper v1.9.4/pty, NDK 27) + Gradle tasks (`ensureNativeBuildTools`/`fetchBuiltinModel`/`fetchFfmpegLibs`/`verifyEmbeddedAi`=BUILD-FAIL); workflow ตัด shell มือ; ลบ scripts คู่มือ; `LOCAL_BUILD.md` ใหม่ = `./gradlew assembleDebug` คำสั่งเดียว |
| CP-147 | Qwen3-4B brain + URL/Intent + tool schemas + browser setup | TODO → DONE | model swap 0.5B→Qwen3-4B (2.5GB, `/no_think`, stripThinking) + `UrlResolver`/`IntentResolver`/`SearchEngines` + `StandardToolSchemas` 38 tools (bound/unbound จริง) + executor `search`/`info`/`current` + `AgentLoop` verify hook + new-tab sheet แทน auto-DDG + `docs/product/*matrix.md` + โมเดล 5 parts (AGP 2GB cap) |
| CP-148 | LLM re-plan เต็มระบบ (วางแผน/แก้ตัวเอง/จำ) | TODO → DONE | `BuiltinLlmProvider` (Qwen3 ฝังเครื่องพูด LlmProvider) + `LlmPlanner`/`LlmRePlanner` (JSON plan ผ่าน resolver เดิม) + `PlanDiagnoser` (§32) + `AgentLoop`/`BootstrapOrchestrator` self-correction (retry/fix/replan/handoff) + `AgentMemory` 6 scope + redaction + `LlmVerifier`; เสียบจริง: CascadePlanner + tool-brain cascade + rePlanner ใน ServiceLocator |
