...
/**
 * CP-147: model-parts protocol. AGP cannot buffer a single 2.5GB asset
 * (`compressDebugAssets`: "Required array size too large"), so Gradle ships
 * the model as 512MiB parts + manifest and the app joins them on first launch
...
object BuiltinModelParts {
    ...
    data class Manifest(val parts: Int, val totalBytes: Long)
